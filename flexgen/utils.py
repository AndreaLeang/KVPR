...
def write_csv_log(filename, args, model_size, cache_size, hidden_size,
        gpu_peak_mem, projected, prefill_latency, prefill_throughput,
        decode_latency, decode_throughput, total_latency, total_throughput, recompute_len):

    column_names = ["model size", "gpu_batch_size", "num_gpu_batches", "prompt_len",
    "gen_len", "recompute_len", "compress_weight", "load_partial_weight", "cache_size",
    ...
    with open(filename, mode="a", newline="\n") as file:
        ...
        data = [f"{model_size/GB:.3f}", args.gpu_batch_size, args.num_gpu_batches, args.prompt_len,
            args.gen_len, recompute_len, args.compress_weight, args.load_partial_weight, f"{cache_size/GB:.3f}",
            f"{hidden_size/GB:.3f}", f"{gpu_peak_mem/GB:.3f}", f"{prefill_latency:.3f}",
            f"{prefill_throughput:.3f}", f"{decode_latency:.3f}", f"{decode_throughput:.3f}",
            f"{total_latency:.3f}", f"{total_throughput:.3f}"]
...
def test_get_optimal_split_point():
    execute_gen_lens = [256, 512, 1024]
    generate_lens = [32, 128]
    gpu_batch_sizes = [32]
    input_dim = 5120
    
    for execute_gen_len in execute_gen_lens:
        for generate_len in generate_lens:
            for gpu_batch_size in gpu_batch_sizes:
                optimal_split_point = get_optimal_split_point(execute_gen_len + generate_len, gpu_batch_size, input_dim)
                print(f"optimal_split_point for execute_gen_len {execute_gen_len} generate_len {generate_len} gpu_batch_size {gpu_batch_size}: {optimal_split_point}")


if __name__ == "__main__":
    test_get_optimal_split_point()

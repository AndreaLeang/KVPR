...
class TorchDevice:
    ...
    def init_attention_compute_workspace(self, config, task, policy):
        ...
        if not policy.compress_cache:
            ...
            for i in range(1 if policy.sep_layer else 2):
                ...
                # only allocate just enough CPU buffer for computation; different from k_home
                ratio = b * n_head * self.len_cpu // (self.len_gpu + self.len_cpu)
                # print(f"init_cache_recompute_one_gpu_batch ratio: {ratio}")
    ...
    def mha_gen(self, inputs, attention_mask, w_q, b_q, w_k, b_k, w_v, b_v,
        ...
        # _, compute_s, _ = inputs_compute.shape
        src_s = attention_mask.shape[1]
        head_dim = h // n_head
        scaling = head_dim ** -0.5

        # print(f"b: {b} tgt_s: {tgt_s} h: {h} compute_s: {compute_s} src_s: {src_s} n_head: {n_head} head_dim: {head_dim}")

        hidden = F.layer_norm(inputs.data, (h,), weight=w_ln.data, bias=b_ln.data)
        # hidden_compute = F.layer_norm(inputs_compute.data, (h,), weight=w_ln.data, bias=b_ln.data)

        # shape: (b, 1, h)
        q = F.linear(hidden, w_q.data, bias=b_q.data) * scaling
        k = F.linear(hidden, w_k.data, bias=b_k.data)
        v = F.linear(hidden, w_v.data, bias=b_v.data)
        # k_compute = F.linear(hidden_compute, w_k.data, bias=b_k.data)
        # v_compute = F.linear(hidden_compute, w_v.data, bias=b_v.data)
        # print(f"k_compute.shape: {k_compute.shape}")

        # shape: (b, 1, n_head, head_dim)
        q = q.view(b, tgt_s, n_head, head_dim)
        k = k.view(b, tgt_s, n_head, head_dim)
        v = v.view(b, tgt_s, n_head, head_dim)
        # k_compute = k.view(b, compute_s, n_head, head_dim)
        # v_compute = v.view(b, compute_s, n_head, head_dim)
        # print(f"k_compute.shape after view: {k_compute.shape}")
        ...
        # k_compute_new = k.permute(1, 0, 2, 3).reshape(compute_s, b * n_head, head_dim)
        # v_compute_new = v.permute(1, 0, 2, 3).reshape(compute_s, b * n_head, head_dim)
        # print(f"k_compute_new.shape: {k_compute_new.shape}")

        # concatenate k_new and k_compute_new
        # result = torch.cat((tensor1, tensor2), dim=0)
        # concatenate v_new and v_compute_new

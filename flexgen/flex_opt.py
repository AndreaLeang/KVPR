...
class InputEmbed:
    ...
    def load_weight_1(self, weight_home, weight_read_buf, k):
        self.load_weight(weight_home, weight_read_buf, k)

    def load_weight_2(self, weight_home, weight_read_buf, k):
        pass
    ...
    def load_cache(self, cache_home, cache_read_buf, i, cpu_cache_read_buf):
        pass  # do nothing
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k, hidden_compute_read_buf, weight_read_buf_2, cpu_cache_read_buf):
...
class SelfAttention:
    def __init__(self, config, env, policy, layer_id, recompute_len, load_partial_weight, cpu_gpu_compute):
        self.config = config
        self.env = env
        ...
        self.load_partial_weight = load_partial_weight
    ...
    def load_weight_1(self, weight_home, weight_read_buf, k):
        ...
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((w_k.smart_copy(dst1), b_k.smart_copy(dst2),
                w_v.smart_copy(dst1), b_v.smart_copy(dst2), w_ln.smart_copy(dst2), b_ln.smart_copy(dst2)))
    
    def load_weight_2(self, weight_home, weight_read_buf_2, k):
        w_q, b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf_2.store((
                w_q.smart_copy(dst1), b_q.smart_copy(dst2),
                w_out.smart_copy(dst1), b_out.smart_copy(dst2)))
    ...
    def init_hidden_one_gpu_batch(self, hidden_compute_home):
        ...
        hidden = device.init_hidden_compute_one_gpu_batch(self.config, self.recompute_len, self.policy)
        hidden_compute_home.store(hidden)
    ...
    def load_cache(self, cache_home, cache_read_buf, i, cpu_cache_read_buf):
        ...
        if path == 0:  # Direct copy
            ...
            # with GPU recomputation, only (s-l, b * n_head, head_dim) KV cache needs to be transferred
        ...
        elif path == 3: # Both GPU and CPU compute MHA, and only CPU stores KV cache.
            # The caches are stored on CPU only.
            # First copy partial KV cache from CPU to GPU; then GPU computes MHA;
            # meanwhile CPU also computes MHA, and sends the result to GPU.
            # shape: (s, b * n_head, head_dim)
            # with GPU recomputation, only (s-l, b * n_head, head_dim) KV cache needs to be transferred
            gpu_indices = (slice(self.recompute_len, self.task.prompt_len + i),
                       slice(0, self.env.cpu.len_gpu))
            # print(f"gpu_indices: {gpu_indices}")
            k_buf, v_buf = dst.next_attention_compute_workspace()
            # print(f"k_buf.shape: {k_buf.shape}")
            cpu_indices = (slice(0, self.task.prompt_len + i),
                       slice(self.env.cpu.len_gpu, k_home.shape[1]))
            # print(f"cpu_indices: {cpu_indices}")
            dst_cpu_indices = (slice(0, self.task.prompt_len + i),
                       slice(0, k_buf.shape[1]))
            # print(f"dst_cpu_indices: {dst_cpu_indices} k_buf.shape[1]: {k_buf.shape[1]}")
    ...
    def load_hidden_compute(self, hidden_compute_home, hidden_compute_read_buf, i):
        if i == 0:  # prefill, no cache
            return
        hidden_activation = hidden_compute_home.val
        
        # dim3_size = hidden_activation.shape[2] * self.env.cpu.len_gpu // (self.env.cpu.len_gpu + self.env.cpu.len_cpu)
        # print(f"dim3_size: {dim3_size}")

        # indices = (slice(0, hidden_activation.shape[0]), slice(0, hidden_activation.shape[1]), slice(0, dim3_size))
        indices = (slice(0, hidden_activation.shape[0]), slice(0, hidden_activation.shape[1]))
        # print(f"hidden_activation.shape: {hidden_activation.shape}")
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if self.load_partial_weight:
            if k == self.policy.num_gpu_batches - 1:
                # Clear the weight_read_buf if it is the last gpu batch
                ((w_k, donate[4]), (b_k, donate[5]), (w_v, donate[6]), (b_v, donate[7]), (w_ln, donate[10]), (b_ln, donate[11])) = weight_read_buf.pop()
            else:
                ((w_k, _), (b_k, _), (w_v, _), (b_v, _), (w_ln, _), (b_ln, _)) = weight_read_buf.val
        else:
            if k == self.policy.num_gpu_batches - 1:
                # Clear the weight_read_buf if it is the last gpu batch
        ...
        if self.recompute_len > 0 and i > 0:
            if w_k.device.device_type == DeviceType.COMPRESSED:
                w_k_compute = w_k.device.decompress(w_k)
                w_v_compute = w_v.device.decompress(w_v)
            else:
                w_k_compute = w_k
                w_v_compute = w_v
            ...
            if self.policy.compress_cache:
                ...
                # hidden_compute = hidden_compute.device.decompress(hidden_compute)[:self.recompute_len]
                hidden_compute = hidden_compute.device.decompress(hidden_compute)
            ...
            k_compute_new = k_compute.permute(1, 0, 2, 3).reshape(compute_s, b * n_head, head_dim)
            v_compute_new = v_compute.permute(1, 0, 2, 3).reshape(compute_s, b * n_head, head_dim)
            # k_compute_new = k_compute.permute(1, 0, 2, 3).reshape(compute_s, dim2_size, head_dim)
            # v_compute_new = v_compute.permute(1, 0, 2, 3).reshape(compute_s, dim2_size, head_dim)
            # k_compute_new = k_compute.permute(1, 0, 2, 3).reshape(compute_s, dim2_size, head_dim)
            # v_compute_new = v_compute.permute(1, 0, 2, 3).reshape(compute_s, dim2_size, head_dim)
            if self.cpu_gpu_compute:
                ...
                # print(f"k_compute_new.shape: {k_compute_new.shape}")
                # print(f"k_compute_new[:, dim2_size, :].shape: {k_compute_new[:, :dim2_size, :].shape}")
                k_cache_expanded = torch.empty((self.task.prompt_len + i, dim2_size, head_dim), dtype=torch.float16, device="cuda:0")
                v_cache_expanded = torch.empty((self.task.prompt_len + i, dim2_size, head_dim), dtype=torch.float16, device="cuda:0")
                k_cache_expanded[:compute_s].copy_(k_compute_new[:, :dim2_size, :])
                v_cache_expanded[:compute_s].copy_(v_compute_new[:, :dim2_size, :])
                # k_cache_expanded[:compute_s].copy_(k_compute_new)
                # v_cache_expanded[:compute_s].copy_(v_compute_new)
            else:
                k_cache_expanded = torch.empty((self.task.prompt_len + i, b * n_head, head_dim), dtype=torch.float16, device="cuda:0")
                v_cache_expanded = torch.empty((self.task.prompt_len + i, b * n_head, head_dim), dtype=torch.float16, device="cuda:0")
                k_cache_expanded[:compute_s].copy_(k_compute_new)
                v_cache_expanded[:compute_s].copy_(v_compute_new)
        ...
        if self.load_partial_weight:
            if k == self.policy.num_gpu_batches - 1:
                ((w_q, donate[2]), (b_q, donate[3]), (w_out, donate[8]), (b_out, donate[9])) = weight_read_buf_2.pop()
            else:
                ((w_q, _), (b_q, _), (w_out, _), (b_out, _)) = weight_read_buf_2.val
        ...
        if i == 0:  # prefill
            ...
            if self.cpu_gpu_compute:
                ...
                b, tgt_s, h1 = inputs.shape
                src_s = mask.shape[1]
                head_dim = h1 // n_head
                scaling = head_dim ** -0.5

                hidden_var = F.layer_norm(inputs.data, (h1,), weight=w_ln.data, bias=b_ln.data)

                # shape: (b, 1, h)
                q = F.linear(hidden_var, w_q.data, bias=b_q.data) * scaling
                k = F.linear(hidden_var, w_k.data, bias=b_k.data)
                v = F.linear(hidden_var, w_v.data, bias=b_v.data)
                ...
                (cpu_k_cache, _), (cpu_v_cache, _) = cpu_cache_read_buf.pop()
                value_cpu = self.compute._mixed_cpu_attention(q, cpu_k_cache, cpu_v_cache,
                    k_new, v_new, mask.data, b, src_s, tgt_s,
                    n_head, head_dim)
            ...
            if self.cpu_gpu_compute:
                if self.recompute_len > 0:
                    ...
                    # print(f"k_cache.data.shape: {k_cache.data.shape}")
                    # print(f"k_cache_expanded.shape: {k_cache_expanded.shape}")
                    # print(f"k_cache_expanded[compute_s:].shape: {k_cache_expanded[compute_s:].shape}")
                    k_cache_expanded[compute_s:].copy_(k_cache.data)
                    v_cache_expanded[compute_s:].copy_(v_cache.data)
                    k_cache.data = k_cache_expanded
                    v_cache.data = v_cache_expanded

                value_gpu = self.compute._mixed_gpu_attention(q, k_cache, v_cache,
                    k_new, v_new, mask.data, b, src_s, tgt_s,
                    n_head, head_dim)
                h, new_k_cache, new_v_cache = self.compute.mha_gen_helper(value_cpu, value_gpu, b, tgt_s, h1, w_out, b_out, inputs, mask, donate,
                    self.policy.compress_cache, self.policy.comp_cache_config, src_s, k, v, k_new, v_new)


            if self.recompute_len > 0 and not self.cpu_gpu_compute:
                if self.policy.compress_cache:
                    k_cache = k_cache.device.decompress(k_cache)
                    v_cache = v_cache.device.decompress(v_cache)
                ...
                k_cache.data = k_cache_expanded
                v_cache.data = v_cache_expanded
                # k_cache.shape = k_cache.data.shape
                # v_cache.shape = v_cache.data.shape


            if not self.cpu_gpu_compute:
...
class TransformerLayer:
    ...
    def init_hidden_one_gpu_batch(self, hidden_compute_home):
        self.attention.init_hidden_one_gpu_batch(hidden_compute_home)

    def load_cache(self, cache_home, cache_read_buf, i, cpu_cache_read_buf):
        self.attention.load_cache(cache_home, cache_read_buf, i, cpu_cache_read_buf)
...
class OptLM:
    def __init__(self,
                 ...
                 policy: Policy,
                 recompute_len: int,
                 load_partial_weight: bool,
                 cpu_gpu_compute: bool):
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                layers.append(SelfAttention(self.config, self.env, self.policy, i, self.recompute_len, self.load_partial_weight, self.cpu_gpu_compute))
        ...
        self.load_weight_stream_2 = torch.cuda.Stream()
        ...
        self.cpu_cache_read_buf = array_2d(num_layers, num_gpu_batches, ValueHolder)
        self.cache_write_buf = array_2d(num_layers, num_gpu_batches, ValueHolder)
        self.hidden_compute_home = array_2d(num_layers, num_gpu_batches, ValueHolder)
        self.hidden_compute_read_buf = array_2d(num_layers, num_gpu_batches, ValueHolder)
        # weight[j]
        self.weight_read_buf = array_1d(num_layers, ValueHolder)
        self.weight_read_buf_2 = array_1d(num_layers, ValueHolder)
        # attention_mask[k]
    ...
    def load_weight(self, i, j, k, overlap=False):
        # Handle corner cases
        ...
        if overlap:
            if self.load_partial_weight:
                with torch.cuda.stream(self.load_weight_stream):
                    self.layers[j].load_weight_1(self.weight_home[j], self.weight_read_buf[j], k)
                with torch.cuda.stream(self.load_weight_stream_2):
                    self.layers[j].load_weight_2(self.weight_home[j], self.weight_read_buf_2[j], k)
            else:
                with torch.cuda.stream(self.load_weight_stream):
    ...
    def init_hidden(self, j, k):
        self.layers[j].init_hidden_one_gpu_batch(self.hidden_compute_home[j][k])

    def load_cache(self, i, j, k, overlap=False):
        # Handle corner cases
        if i == 0:  # prefill, no cache
            return
        ...
        if overlap:
            with torch.cuda.stream(self.load_cache_stream):
                self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i, self.cpu_cache_read_buf[j][k])
        else:
            self.layers[j].load_cache(self.cache_home[j][k], self.cache_read_buf[j][k], i, self.cpu_cache_read_buf[j][k])
    
    def load_hidden_compute(self, i, j, k, overlap=False):
        if i == 0:
            return
        if self.recompute_len == 0:
            return
    ...
    def delete_hidden_compute(self, j, k):
        v = self.hidden_compute_home[j][k].pop()
        if v:
            v.delete()
    ...
    def compute_layer(self, i, j, k):
        ...
        self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
            ...
            self.weight_read_buf_2[j], self.cpu_cache_read_buf[j][k])
    ...
    def generate(self,
        ...
        for j in range(num_layers):
            for k in range(num_gpu_batches):
                ...
                self.cpu_cache_read_buf[j][k].clear()
                self.cache_write_buf[j][k].clear()
                self.hidden_compute_home[j][k].clear()
                self.hidden_compute_read_buf[j][k].clear()
        for j in range(num_layers):
            self.weight_read_buf[j].clear()
            self.weight_read_buf_2[j].clear()
        for k in range(num_gpu_batches):
            self.attention_mask[k].clear()
        self.hidden = array_3d(gen_len, num_layers, num_gpu_batches, ValueHolder)

        print(f"gen_len: {gen_len} num_layers: {num_layers} num_gpu_batches: {num_gpu_batches}")
    ...
    def generation_loop_normal(self):
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    self.load_hidden_compute(i, j, k)
    ...
    def generation_loop_debug_normal(self):
        ...
        timers("load_hidden_prefill").reset()
        timers("load_hidden_decoding").reset()
        timers("store_hidden_prefill").reset()
        timers("store_hidden_decoding").reset()
        timers("load_hidden_compute_prefill").reset()
        timers("load_hidden_compute_decoding").reset()
        ...
        for i in range(self.execute_gen_len):
            if i == 0:
                ...
                load_hidden_timer = timers("load_hidden_prefill")
                store_hidden_timer = timers("store_hidden_prefill")
                load_hidden_compute_timer = timers("load_hidden_compute_prefill")
            else:
                ...
                load_hidden_timer = timers("load_hidden_decoding")
                store_hidden_timer = timers("store_hidden_decoding")
                load_hidden_compute_timer = timers("load_hidden_compute_decoding")
            ...
            for j in range(self.num_layers):
                ...
                for k in range(self.num_gpu_batches):
                    load_hidden_compute_timer.start(self.sync)
                    self.load_hidden_compute(i, j, k)
                    load_hidden_compute_timer.stop(self.sync)
        ...
        for stage in ["prefill", "decoding"]:
            for func in ["load_cache", "store_cache", "compute_layer", "load_hidden", "store_hidden", "load_hidden_compute"]:
                name = func + "_" + stage
    ...
    def generation_loop_overlap_single_batch(self):
        ...
        # Prologue
        for k in range(self.num_gpu_batches):
            self.load_weight(0, 0, k, overlap)
        self.sync()
        ...
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                self.load_weight(i, j+1, 0, overlap)
                self.load_hidden_compute(i, j+1, 0, overlap)
                self.load_cache(i, j+1, 0, overlap)
                self.load_hidden(i, j, 0)
                self.compute_layer(i, j, 0)
                self.store_cache(i, j-1, 0, overlap)
    ...
    def generation_loop_overlap_multi_batch(self):
        ...
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                for k in range(self.num_gpu_batches):
                    self.load_weight(i, j+1, k, overlap)
                    self.load_hidden_compute(i, j, k+1, overlap)
                    self.load_cache(i, j, k+1, overlap)
                    self.store_hidden(i, j, k-1)
                    self.load_hidden(i, j, k+1)
                    self.compute_layer(i, j, k)
                    self.store_cache(i, j, k-1, overlap)
                    self.sync()
            timers("generate").stop()
...
def run_flexgen(args):
    ...
    f"gen-len: {args.gen_len}\ngpu-batch-size: {args.gpu_batch_size}\nnum-gpu-batches: {args.num_gpu_batches}\n"
     f"cpu-cache-compute: {args.cpu_cache_compute}\npin_memory: {args.pin_weight}\noverlap: {args.overlap}\nkv_partial: {args.kv_partial}\n"
     f"compress-weight: {args.compress_weight}\nload_partial_weight: {args.load_partial_weight}")
    ...
    for i in range(args.num_test_runs):
        ...
        print(f"recompute_len: {recompute_len}")
        model = OptLM(opt_config, env, args.path, policy, recompute_len, args.load_partial_weight, args.cpu_gpu_compute)
        ...
        try:
            ...
            trace_filename = f"L40s_facebook_m_zigzagnum_{args.num_gpu_batches}_batchSize_{args.gpu_batch_size}_trace.json"
...
def add_parser_arguments(parser):
    ...
    parser.add_argument("--csv-file", type=str, default="test.csv")
    parser.add_argument("--no-log", action="store_true")
    parser.add_argument("--verbose", type=int, default=2)
    parser.add_argument("--overlap", action="store_true")
    parser.add_argument("--load-partial-weight", action="store_true")
...
if __name__ == "__main__":
    ...
    args.csv_file = "optimal_latency_benchmark_L40s_tot_64_all_cpu.csv"
    args.overlap = True

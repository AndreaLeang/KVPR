...
class TorchCompressedDevice:
    ...
    def init_hidden_compute_one_gpu_batch(self, config, recompute_len, policy):
        ...
        hidden_compute_compress_config = CompressionConfig(num_bits=4, group_size=64,
                                        group_dim=0, symmetric=False)
        hidden_compute = self.allocate(shape, np.float16,
            comp_config=hidden_compute_compress_config, pin_memory=pin_memory)

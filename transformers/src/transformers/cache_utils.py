...
class OffloadedStaticCache(StaticCache):
    ...
    def __init__(
        ...
        offloaded_cache_shape = (self.batch_size, num_key_value_heads, self.max_cache_len - self.recompute_len, head_dim)
        cache_shape = (self.batch_size, num_key_value_heads, self.max_cache_len, head_dim)
        recomputed_activations_shape = (self.batch_size, self.recompute_len, config.hidden_size)
        ...
        # Create offloaded CPU activations.
        ...
        for i in range(config.num_hidden_layers):
            ...
            # only allocate just enough kv cache to avoid copy-slicing opeartions
            key_cache, value_cache = self._create_key_value_cache_tensors(offloaded_cache_shape, device)
            tmp_activations = self._create_recomputed_activations(recomputed_activations_shape, device)

            self.key_cache.append(key_cache)
            self.value_cache.append(value_cache)
            if self.recompute_len > 0:
                self.activations.append(tmp_activations)
        ...
        if self.recompute_len > 0:
            for i in range(2):
                recomputed_activations = self._create_recomputed_activations(recomputed_activations_shape, self.device)
                self._device_recomputed_activations.append(recomputed_activations)
    ...
    def update(
        ...
        if self.recompute_len > 0:
            if hidden_states.shape[1] == 1:
                if layer_idx == 0:
                    recomputed_key_states = self._shape(k_proj(recomputed_activations), -1, self.batch_size)
                    recomputed_value_states = self._shape(v_proj(recomputed_activations), -1, self.batch_size)
        ...
        if cache_position is None:
            ...
            if layer_idx != 0:
                ...
                if seq_len > 1:
                    ...
                    source_position = cache_position
                    source_position = source_position.to(self.offload_device)
                    key_states = key_states.to(self.offload_device)
                    value_states = value_states.to(self.offload_device)
                ...
                try:
                    if seq_len > 1:
                        self.key_cache[layer_idx].index_copy_(2, target_position, key_states[:, :, source_position, :])
                        self.value_cache[layer_idx].index_copy_(2, target_position, value_states[:, :, source_position, :])
                    else:
                        self.key_cache[layer_idx].index_copy_(2, target_position, key_states)
                        self.value_cache[layer_idx].index_copy_(2, target_position, value_states)
                except NotImplementedError:
                    # The operator 'aten::index_copy.out' is not currently implemented for the MPS
                    # device.
    ...
    def _create_recomputed_activations(
        self, shape: Tuple[int, ...], device: torch.device
    ) -> torch.Tensor:
    ...
    def _prefetch_layer_in_context(self, layer_idx: int) -> None:
        ...
        if self.recompute_len > 0:
            self._device_recomputed_activations[layer_idx & 1].copy_(self.activations[layer_idx], non_blocking=True)

...
class OPTAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_id, cache_kwargs, hidden_states, self.k_proj, self.v_proj)
        else:
            # self_attention
            key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
            value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, layer_id: int):
        super().__init__()
        self.embed_dim = config.hidden_size

        self.self_attn = OPT_ATTENTION_CLASSES[config._attn_implementation](config=config, is_decoder=True, layer_id=layer_id)
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.layers = nn.ModuleList([OPTDecoderLayer(config, layer_id=i) for i in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        causal_attention_mask = self._update_causal_mask(

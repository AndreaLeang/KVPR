...
from flexgen.utils import (GB, project_decode_latency,
    write_benchmark_log, get_optimal_split_point)
...
def get_ds_opt_model(model_name, dtype, cpu_offload, disk_offload, offload_dir,
    ...
    model = OPTForCausalLM.from_pretrained(
        dummy_weights or model_name, torch_dtype=dtype, attn_implementation="eager")
    model = model.eval()
...
def get_hf_opt_model(model_name, dtype, cpu_offload, disk_offload, offload_dir,
    ...
    model = OPTForCausalLM.from_pretrained(dummy_weights or model_name,
        ...
        offload_folder=offload_dir, **kwargs, attn_implementation="eager")
...
def run_generation(model_name, batch_size, prompt_len, gen_len, cut_gen_len,
    ...
    column_names = ["model size", "batch_size", "prompt_len",
    "gen_len", "cache_size", "recompute_len",
    "hidden_size", "gpu_peak_mem", "prefill_latency", "prefill_throughput",
    "decode_latency", "decode_throughput", "total_latency", "total_throughput"]

    with open(args.csv_file, mode="a", newline="\n") as file:
        writer = csv.writer(file, delimiter='\t')
        if os.stat(args.csv_file).st_size == 0:
            writer.writerow(column_names)
        data = [f"{opt_config.model_bytes()/GB:.3f}", args.batch_size, args.prompt_len,
            args.gen_len, f"{cache_size/GB:.3f}", args.recompute_len,
